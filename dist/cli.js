...
async function cmdHelp(toolName) {
    ...
    lines.push(`\nExample:\n  wpstudio ${tool.name.replace(/_/g, "-")} ${Object.keys(schema?.properties ?? {}).slice(0, 2)
        .map(k => `--${k} <value>`).join(" ")}\n`);
    console.log(lines.join("\n"));
}
// ─── Main: call a tool ────────────────────────────────────────────────────────
async function cmdCall(toolName, toolArgs, asJson) {
    const mcpName = cliToMcp(toolName);
    ...
    let result;
    try {
        result = await withClient(c => c.callTool({ name: mcpName, arguments: toolArgs }));
    }
    catch (err) {
