...
import { z } from "zod";
import { getSites, readCliConfig, readSharedConfig, STUDIO_HOME, STUDIO_SITES_ROOT, DAEMON_SOCKET, CLI_CONFIG_PATH, SHARED_CONFIG_PATH, APP_CONFIG_PATH, } from "../studio-config.js";
export function registerSiteRegistryTools(server) {
    // ── studio_registry ───────────────────────────────────────────────────────
    server.tool("studio_registry", "Return the full Studio site registry from ~/.studio/cli.json — site IDs, names, " +
        "disk paths, ports, PHP versions, WordPress versions, and auto-login URLs.", {}, async () => {
    ...
    // ── studio_auth_status ────────────────────────────────────────────────────
    server.tool("studio_auth_status", "Show WordPress.com OAuth token status from ~/.studio/shared.json. " +
        "The actual access token is never returned.", {}, async () => {
    ...
    // ── studio_disk_usage ─────────────────────────────────────────────────────
    server.tool("studio_disk_usage", "Report disk usage for all Studio sites.", {
        ...
        const lines = sites.map(s => {
            if (!fs.existsSync(s.path))
                return `${s.name}: path not found (${s.path})`;
    ...
    // ── studio_site_health ────────────────────────────────────────────────────
    server.tool("studio_site_health", "HTTP health check on a running Studio site — returns status code, response time, and WP headers.", {
        site: z.string().describe("Site name or absolute site path"),
    }, async ({ site }) => {
        const siteData = getSites().find(s => s.name.toLowerCase() === site.toLowerCase() || s.path === site);
        if (!siteData) {
            return { content: [{ type: "text", text: `Site "${site}" not found in registry.` }], isError: true };
        }
    ...
    server.tool("studio_daemon_status", "Check whether the Studio process manager daemon is running by probing its named pipe / Unix socket.", {}, async () => {
    ...
    // ── studio_config_dump ────────────────────────────────────────────────────
    server.tool("studio_config_dump", "Dump raw Studio config files — passwords and token values are always redacted.", {
        ...
        const redact = (obj) => {
            ...
            for (const key of Object.keys(copy)) {
                if (/password|token|key|secret|salt/i.test(key))
                    copy[key] = "[REDACTED]";
                else
        ...
        if (file === "cli" || file === "all")
            load("CLI Config", CLI_CONFIG_PATH);
        if (file === "shared" || file === "all")
            load("Shared Config", SHARED_CONFIG_PATH);
        if (file === "app" || file === "all")
            load("App Config", APP_CONFIG_PATH);
