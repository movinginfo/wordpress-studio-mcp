...
function studioWp(sitePath, wpArgs) {
    const result = spawnSync("studio", ["wp", "--path", sitePath, ...wpArgs], {
        encoding: "utf-8",
...
export function registerWpCliTools(server) {
    // ── wpcli_plugin_list ─────────────────────────────────────────────────────
    server.tool("wpcli_plugin_list", "List all installed plugins for a Studio site with status, version, and update info.", {
        ...
        return { content: [{ type: "text", text: ok ? stdout : stderr }], isError: !ok };
    });
    // ── wpcli_plugin_install ──────────────────────────────────────────────────
    server.tool("wpcli_plugin_install", "Install (and optionally activate) a WordPress plugin from wordpress.org.", {
        site: z.string().describe("Site name or absolute site path"),
    ...
    server.tool("wpcli_plugin_deactivate", "Deactivate a WordPress plugin on a Studio site.", {
        site: z.string().describe("Site name or absolute site path"),
    ...
    // ── wpcli_theme_list ──────────────────────────────────────────────────────
    server.tool("wpcli_theme_list", "List all installed themes for a Studio site.", {
        site: z.string().describe("Site name or absolute site path"),
    }, async ({ site }) => {
        const siteData = findSite(site);
        const sitePath = siteData?.path ?? path.join(STUDIO_SITES_ROOT, site);
    ...
    server.tool("wpcli_theme_activate", "Activate a theme on a Studio site.", {
        site: z.string().describe("Site name or absolute site path"),
    ...
    // ── wpcli_user_list ───────────────────────────────────────────────────────
    server.tool("wpcli_user_list", "List all WordPress users for a Studio site.", {
        site: z.string().describe("Site name or absolute site path"),
    }, async ({ site }) => {
        const siteData = findSite(site);
        const sitePath = siteData?.path ?? path.join(STUDIO_SITES_ROOT, site);
    ...
    // ── wpcli_create_admin ────────────────────────────────────────────────────
    server.tool("wpcli_create_admin", "Create a new administrator user on a Studio site.", {
        site: z.string().describe("Site name or absolute site path"),
    ...
    server.tool("wpcli_search_replace", "Run a database search-replace on a Studio site (useful for URL migration). " +
        ...
        if (dry_run)
            args.push("--dry-run");
        if (skip_tables?.length)
            args.push(`--skip-tables=${skip_tables.join(",")}`);
        const { stdout, stderr, ok } = studioWp(sitePath, args);
        return { content: [{ type: "text", text: ok ? stdout : stderr }], isError: !ok };
    });
    // ── wpcli_cache_flush ─────────────────────────────────────────────────────
    server.tool("wpcli_cache_flush", "Flush WordPress object cache, rewrite rules, and transients for a Studio site.", {
        site: z.string().describe("Site name or absolute site path"),
    }, async ({ site }) => {
        const siteData = findSite(site);
        const sitePath = siteData?.path ?? path.join(STUDIO_SITES_ROOT, site);
    ...
    // ── wpcli_core_update ─────────────────────────────────────────────────────
    server.tool("wpcli_core_update", "Update WordPress core on a Studio site.", {
        site: z.string().describe("Site name or absolute site path"),
    ...
    // ── wpcli_run ─────────────────────────────────────────────────────────────
    server.tool("wpcli_run", "Run any arbitrary WP-CLI command on a Studio site via `studio wp`. " +
