 ...
 */
function wpConfigUpdate(wpConfigPath, constant, value) {
    ...
    const existRe = new RegExp(`(define\\s*\\(\\s*['"]${constant}['"]\\s*,\\s*)([^)]+)(\\)\\s*;)`, "m");
...
export function registerAdminTools(server) {
    // ── wp_config_set ─────────────────────────────────────────────────────────
    server.tool("wp_config_set", "Add, update, or remove a constant in wp-config.php without rewriting the whole file. " +
        ...
        if (!fs.existsSync(wpConfigPath)) {
            return {
                content: [{ type: "text", text: `wp-config.php not found at ${wpConfigPath}` }],
                isError: true,
            };
        }
        try {
            ...
            return { content: [{ type: "text", text: result.message + suffix }] };
        }
        catch (err) {
            return {
    ...
    // ── wp_security_audit ─────────────────────────────────────────────────────
    server.tool("wp_security_audit", "Run a security checklist against a Studio site based on the WordPress Advanced " +
        ...
        const getConst = (name) => {
            const m = config.match(new RegExp(`define\\s*\\(\\s*['"]${name}['"]\\s*,\\s*([^)]+)\\)`, "i"));
            ...
            for (const dir of [siteRoot, path.join(siteRoot, "wp-content")]) {
                ...
                for (const entry of fs.readdirSync(dir)) {
                    try {
                        ...
                        if (stat.mode & 0o002)
                            worldWritable.push(path.relative(siteRoot, path.join(dir, entry)));
                    }
                    catch { /* skip */ }
                }
            }
    ...
    // ── wp_php_info ───────────────────────────────────────────────────────────
    server.tool("wp_php_info", "Get PHP runtime configuration for a Studio site: version, memory limits, upload limits, " +
        ...
        const phpCode = "echo json_encode([" +
        ...
        if (!ok) {
            return {
                content: [{ type: "text", text: stderr || "Failed to get PHP info. Is the site running?" }],
                isError: true,
            };
        }
        try {
    ...
    // ── wpcli_db_backup ───────────────────────────────────────────────────────
    server.tool("wpcli_db_backup", "Export the WordPress database to a .sql file. " +
        "Defaults to ~/Studio/backups/{site}-{YYYY-MM-DD}.sql. " +
        "Always run before upgrades, migrations, or destructive operations.", {
        site: z.string().describe("Site name or absolute site path"),
        output_path: z.string().optional().describe("Absolute output path for the .sql file. " +
        ...
        if (ok) {
            const size = fs.existsSync(outFile)
                ...
                : "unknown";
            return {
                content: [{ type: "text", text: `✓ Database exported\n  Path: ${outFile}\n  Size: ${size}` }],
            };
        }
        return { content: [{ type: "text", text: stderr || "DB export failed." }], isError: true };
    });
    // ── wpcli_cron_list ───────────────────────────────────────────────────────
    server.tool("wpcli_cron_list", "List all scheduled WordPress cron events for a Studio site — hook name, " +
        ...
        return {
            content: [{ type: "text", text: ok ? (stdout || "No cron events found.") : stderr }],
            isError: !ok,
        };
    });
    // ── wpcli_update_all ──────────────────────────────────────────────────────
    server.tool("wpcli_update_all", "Update all plugins and/or themes on a Studio site to their latest available versions. " +
        ...
        if (what === "plugins" || what === "both")
            run("plugin");
        if (what === "themes" || what === "both")
            run("theme");
        return { content: [{ type: "text", text: lines.join("\n") }] };
    });
}
