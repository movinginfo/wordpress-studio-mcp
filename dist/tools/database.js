...
import { z } from "zod";
import { findSite, getSqlitePath, STUDIO_SITES_ROOT, } from "../studio-config.js";
function openDb(dbPath) {
    return new DatabaseSync(dbPath);
}
export function registerDatabaseTools(server) {
    // ── db_list_tables ────────────────────────────────────────────────────────
    server.tool("db_list_tables", "List all tables in the SQLite database for a Studio site.", {
    ...
    // ── db_describe_table ─────────────────────────────────────────────────────
    server.tool("db_describe_table", "Show column names, types, and constraints for a table in a Studio site's SQLite DB.", {
        site: z.string().describe("Site name or absolute site path"),
        ...
        const dbPath = getSqlitePath(siteRoot);
        if (!fs.existsSync(dbPath)) {
            return { content: [{ type: "text", text: `SQLite DB not found at: ${dbPath}` }], isError: true };
        }
        const db = openDb(dbPath);
        try {
            const validTables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all().map(r => r.name);
            ...
            const rows = cols.map(c => `  ${c.pk ? "🔑" : "  "} ${c.name.padEnd(30)} ${c.type.padEnd(15)} ${c.notnull ? "NOT NULL" : "NULLABLE"}`);
    ...
    // ── db_query ──────────────────────────────────────────────────────────────
    server.tool("db_query", "Execute a read-only SELECT query on a Studio site's SQLite database.", {
        site: z.string().describe("Site name or absolute site path"),
        ...
        if (!trimmed.startsWith("SELECT") && !trimmed.startsWith("WITH")) {
            return {
                content: [{ type: "text", text: "Only SELECT / WITH queries are allowed here. Use db_execute for writes." }],
                isError: true,
            };
        }
        const siteData = findSite(site);
        ...
        try {
            ...
            const body = rows.map(r => Object.values(r).map(v => v === null ? "NULL" : String(v).slice(0, 80)).join(" | "));
    ...
    // ── db_execute ────────────────────────────────────────────────────────────
    server.tool("db_execute", "Execute a write SQL statement (INSERT, UPDATE, DELETE, etc.) on a Studio site's SQLite DB. Changes are immediate.", {
        site: z.string().describe("Site name or absolute site path"),
    ...
    // ── db_export_sql ─────────────────────────────────────────────────────────
    server.tool("db_export_sql", "Export the SQLite database for a Studio site as a SQL dump (schema + INSERT rows).", {
        site: z.string().describe("Site name or absolute site path"),
