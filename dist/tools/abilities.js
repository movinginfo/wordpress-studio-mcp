...
import { z } from "zod";
import { findSite, STUDIO_HOME, } from "../studio-config.js";
// ─── GitHub source URLs ───────────────────────────────────────────────────────
const ABILITIES_API_ZIP = "https://github.com/WordPress/abilities-api/archive/refs/heads/trunk.zip";
const MCP_ADAPTER_ZIP = "https://github.com/WordPress/mcp-adapter/archive/refs/heads/trunk.zip";
// ─── App-password store ───────────────────────────────────────────────────────
const APP_PASSWORDS_FILE = path.join(STUDIO_HOME, "mcp-app-passwords.json");
function loadAppPasswords() {
    try {
        return JSON.parse(fs.readFileSync(APP_PASSWORDS_FILE, "utf-8"));
    }
    catch {
        return [];
    }
}
function saveAppPasswords(entries) {
    fs.writeFileSync(APP_PASSWORDS_FILE, JSON.stringify(entries, null, 2), "utf-8");
}
function getStoredPassword(siteId) {
    return loadAppPasswords().find(e => e.siteId === siteId);
}
function storePassword(entry) {
...
const STUDIO_NODE = (() => {
    ...
    const candidates = [
        path.join(process.env.LOCALAPPDATA ?? path.join(process.env.HOME ?? "~", ".local"), "studio_app", "app-1.7.7", "resources", "bin", "node.exe"),
    ];
...
const STUDIO_CLI_MAIN = (() => {
    const base = path.join(process.env.LOCALAPPDATA ?? path.join(process.env.HOME ?? "~", ".local"), "studio_app", "app-1.7.7", "resources", "cli", "main.js");
...
function studioWp(sitePath, wpArgs) {
    ...
    if (STUDIO_NODE && STUDIO_CLI_MAIN) {
        const r = spawnSync(STUDIO_NODE, ["--experimental-wasm-jspi", STUDIO_CLI_MAIN, "wp", "--path", sitePath, ...wpArgs], { encoding: "utf-8", timeout: 5 * 60 * 1000, windowsHide: true });
        return {
    ...
    const r = spawnSync("studio", ["wp", "--path", sitePath, ...wpArgs], { encoding: "utf-8", timeout: 5 * 60 * 1000, windowsHide: true });
    return {
        stdout: r.stdout?.trim() ?? "",
        stderr: r.stderr?.trim() ?? (r.error?.message ?? ""),
        ok: r.status === 0,
 ...
 */
async function mcpAdapterCall(siteUrl, authB64, toolName, toolArgs) {
    ...
    const postJson = async (body, extra) => {
    ...
    try {
        ...
        await postJson({ jsonrpc: "2.0", method: "notifications/initialized", params: {} }, sessionHeaders).then(r => r.text()).catch(() => null);
        // ── Step 3: tools/call ────────────────────────────────────────────────────
        ...
        if (Array.isArray(content) && content.length > 0) {
            ...
            if (text) {
                try {
                    return { ok: true, result: JSON.parse(text), raw: rpc };
                }
                catch {
                    return { ok: true, result: text, raw: rpc };
                }
            }
        }
...
function discoverAbilitiesViaWpCli(sitePath) {
    const php = `
$abilities = [];
...
function callAbilityViaWpCli(sitePath, abilityName, params) {
...
// ─── Tool registration ────────────────────────────────────────────────────────
export function registerAbilitiesTools(server) {
    // ── wp_mcp_adapter_setup ──────────────────────────────────────────────────
    server.tool("wp_mcp_adapter_setup", "Install and activate the WordPress Abilities API + MCP Adapter plugins " +
    ...
    server.tool("wp_abilities_discover", "List all WordPress abilities registered with mcp.public=true on a Studio site. " +
        ...
        const lines = [`Discovering abilities on: ${siteData.name}`, "─".repeat(50), ""];
        // ── HTTP via MCP adapter ──────────────────────────────────────────────
        if (via === "http" || via === "auto") {
            const stored = getStoredPassword(siteData.id);
            if (!stored && via === "http") {
                return {
                    content: [{ type: "text", text: `No Application Password stored for '${siteData.name}'.\n` +
            ...
            if (stored) {
                ...
                if (result.ok) {
                    const abilities = result.result?.abilities ?? result.result;
        ...
        if (abilities.length === 0) {
            ...
            for (const a of abilities) {
                ...
                if (a.label)
                    lines.push(`    Label:       ${String(a.label)}`);
                if (a.description)
                    lines.push(`    Description: ${String(a.description)}`);
                if (a.type)
                    lines.push(`    Type:        ${String(a.type)}`);
                lines.push("");
            }
        }
        return { content: [{ type: "text", text: lines.join("\n") }] };
    ...
    // ── wp_abilities_info ─────────────────────────────────────────────────────
    server.tool("wp_abilities_info", "Get the full schema for a specific WordPress ability — parameters, return type, " +
        ...
        if (via !== "wpcli") {
            ...
            if (stored) {
                const authB64 = Buffer.from(`${stored.user}:${stored.password}`).toString("base64");
                const result = await mcpAdapterCall(`http://localhost:${siteData.port}`, authB64, "mcp-adapter/get-ability-info", { ability_name: ability });
                if (result.ok) {
...
`.trim();
        ...
        if (!r.ok) {
            return { content: [{ type: "text", text: `WP-CLI eval failed: ${r.stderr}` }], isError: true };
        }
        try {
    ...
    // ── wp_abilities_call ─────────────────────────────────────────────────────
    server.tool("wp_abilities_call", "Execute a WordPress ability with parameters and return the result. " +
        ...
        if (via !== "wpcli") {
            ...
            if (stored) {
                const authB64 = Buffer.from(`${stored.user}:${stored.password}`).toString("base64");
                const result = await mcpAdapterCall(`http://localhost:${siteData.port}`, authB64, "mcp-adapter/execute-ability", { ability_name: ability, params });
                if (result.ok) {
        ...
        const result = callAbilityViaWpCli(siteData.path, ability, params);
        if (!result.ok) {
