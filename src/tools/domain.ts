...
function studioCli(args: string[]): { stdout: string; stderr: string; ok: boolean } {
  const result = spawnSync(
    "studio",
    args,
...
function readHostsFile(): string | null {
  try { return fs.readFileSync(HOSTS_FILE, "utf-8"); }
  catch { return null; }
}

/** Check whether a domain is already in the hosts file */
function domainInHosts(domain: string): boolean {
...
function addDomainToHosts(domain: string, port: number): { ok: boolean; message: string } {
  ...
  try {
    ...
    return {
      ok: false,
      message:
        `Failed to write hosts file (needs admin/elevated privileges).\n` +
...
function removeDomainFromHosts(domain: string): { ok: boolean; message: string } {
  let content = readHostsFile();
  if (content === null) return { ok: false, message: `Cannot read ${HOSTS_FILE}` };
...
function patchCliConfig(siteId: string, domain: string | null): boolean {
  try {
    const raw  = fs.readFileSync(CLI_CONFIG_PATH, "utf-8");
    const cfg  = JSON.parse(raw);
    const site = cfg.sites?.find((s: { id: string }) => s.id === siteId);
...
export function registerDomainTools(server: McpServer): void {
  ...
  server.tool(
    "studio_site_set_domain",
    "Map a real domain name to a local WordPress Studio site so it opens as " +
    ...
    "if it fails the manual command is printed. Restart the site after running this tool.",
    {
      ...
      domain:       z.string().describe(
        "Domain to use, e.g. 'testmysite.com' or 'mysite.local'. Do NOT include http:// or a trailing slash."
      ),
      https:        z.boolean().optional().default(false)
                    .describe("Use https:// (default: false — use http://)"),
      confirmed:    z.boolean().describe(
        "Must be true to apply changes. Set to false to preview only."
      ),
    },
    async ({ site, domain, https: useHttps, confirmed }) => {
      ...
      if (!hostsResult.ok) hasError = true;
  ...
  server.tool(
    "studio_site_remove_domain",
    "Remove a custom domain mapping from a Studio site and revert it back to " +
    ...
    async ({ site, confirmed }) => {
      ...
      if (!oldDomain) {
        return { content: [{ type: "text" as const, text: `Site '${site}' has no custom domain set.` }] };
      }
  ...
  server.tool(
    "studio_domain_list",
    "List all custom domain mappings across Studio sites, show the current hosts file " +
    "Studio block, and check whether each domain resolves correctly.",
    {},
    async () => {
      const cfg    = readCliConfig();
      const sites  = cfg?.sites ?? [];
      const lines: string[] = ["Studio Custom Domain Mappings", "─".repeat(50)];
