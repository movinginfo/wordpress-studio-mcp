...
const REPO_BASE =
  "https://raw.githubusercontent.com/coreyhaines31/marketingskills/main/skills";
...
function skillsDir(scope: "global" | "project"): string {
  return scope === "global" ? GLOBAL_SKILLS_DIR : PROJECT_SKILLS_DIR;
}

function isInstalled(skillName: string, scope: "global" | "project"): boolean {
...
export function registerMarketingTools(server: McpServer): void {
  ...
  server.tool(
    "marketing_skills_list",
    "Browse all 34 marketing skills from https://github.com/coreyhaines31/marketingskills. " +
    ...
    "Once installed, invoke any skill with /skill-name in Claude Code.",
    {
      category: z.enum(["all", ...CATEGORIES] as ["all", ...string[]])
    ...
    async ({ category }) => {
      ...
      const byCategory: Record<string, MarketingSkill[]> = {};
  ...
  server.tool(
    "marketing_skills_install",
    "Download and install marketing skills from https://github.com/coreyhaines31/marketingskills " +
    ...
    "Skills are downloaded fresh from GitHub on each install.",
    {
      skills:    z.union([
                 ...
                 .describe(
                   "Which skills to install: 'all', a category name (cro/copy/seo/paid/analytics/growth/revenue), " +
  ...
  server.tool(
    "marketing_skills_status",
    "Show which marketing skills are installed and where. " +
    "Checks both ~/.claude/skills/ (global) and .claude/skills/ (project). " +
    "Shows install counts per category and lists any missing skills.",
    {},
    async () => {
      ...
      const byCategory: Record<string, { skill: MarketingSkill; global: boolean; project: boolean }[]> = {};
  ...
  server.tool(
    "marketing_skills_context",
    "Guide to set up the product-marketing-context foundation skill. " +
