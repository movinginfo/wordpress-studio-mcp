...
export function registerFilesystemTools(server: McpServer): void {
  ...
  server.tool(
    "fs_read_file",
    "Read any file inside a WordPress Studio site. " +
    "Use site name for 'site', then a relative path like 'wp-content/themes/mytheme/style.css'.",
    {
      ...
      relative_path: z.string().describe(
        "Path relative to the site root, e.g. 'wp-content/themes/mytheme/style.css'"
      ),
    },
    async ({ site, relative_path }) => {
      ...
      if (!fs.existsSync(full)) {
        return { content: [{ type: "text" as const, text: `File not found: ${full}` }], isError: true };
      }
      const stat = fs.statSync(full);
      if (stat.isDirectory()) {
        return { content: [{ type: "text" as const, text: `Path is a directory — use fs_list_dir instead.` }], isError: true };
      }
      if (stat.size > 2 * 1024 * 1024) {
        return {
          content: [{ type: "text" as const, text: `File too large (${(stat.size / 1024).toFixed(1)} KB). Max 2 MB.` }],
          isError: true,
        };
      }
  ...
  server.tool(
    "fs_write_file",
    "Write (create or overwrite) a file inside a WordPress Studio site. " +
    ...
    async ({ site, relative_path, content, create_dirs }) => {
      ...
      if (create_dirs) fs.mkdirSync(path.dirname(full), { recursive: true });
      fs.writeFileSync(full, content, "utf-8");

      return {
        content: [{ type: "text" as const, text: `✓ Written ${content.length} bytes to ${full}` }],
      };
    }
  );
  ...
  server.tool(
    "fs_list_dir",
    "List files and subdirectories inside a WordPress Studio site directory.",
    {
      site:          z.string().describe("Site name or absolute site path"),
      relative_path: z.string().optional().default("").describe(
        "Path relative to site root (default: site root)"
      ),
    ...
    async ({ site, relative_path, recursive }) => {
      ...
      if (!fs.existsSync(full)) {
        return { content: [{ type: "text" as const, text: `Directory not found: ${full}` }], isError: true };
      }
  ...
  server.tool(
    "fs_find_files",
    "Search for files matching a glob pattern inside a WordPress Studio site. " +
  ...
  server.tool(
    "fs_read_wp_config",
    "Read and parse wp-config.php from a Studio site. Passwords and secret keys are redacted.",
    {
      site: z.string().describe("Site name or absolute site path"),
    },
    async ({ site }) => {
      ...
      if (!fs.existsSync(wpConfig)) {
        return { content: [{ type: "text" as const, text: `wp-config.php not found at ${wpConfig}` }], isError: true };
      }

      let content = fs.readFileSync(wpConfig, "utf-8");
      content = content.replace(
        /(define\s*\(\s*'(?:DB_PASSWORD|AUTH_KEY|SECURE_AUTH_KEY|LOGGED_IN_KEY|NONCE_KEY|AUTH_SALT|SECURE_AUTH_SALT|LOGGED_IN_SALT|NONCE_SALT)'\s*,\s*')[^']*/g,
        "$1[REDACTED]"
      );
      return { content: [{ type: "text" as const, text: content }] };
    }
  ...
  server.tool(
    "fs_read_error_log",
    "Read the WordPress debug.log for a Studio site (last N lines).",
    {
      site:  z.string().describe("Site name or absolute site path"),
    ...
    async ({ site, lines }) => {
      ...
      if (!fs.existsSync(logPath)) {
        return { content: [{ type: "text" as const, text: "No debug.log found. Is WP_DEBUG_LOG enabled?" }] };
      }
...
function listDir(dir: string, maxDepth: number, depth: number): string[] {
