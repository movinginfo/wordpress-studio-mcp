...
const VIP_COMPONENTS: Record<string, string[]> = {
...
export function registerVipDesignTools(server: McpServer): void {

  // ── vip_design_tokens ─────────────────────────────────────────────────────

  server.tool(
    "vip_design_tokens",
    "Query the Automattic VIP Design System token library (github.com/Automattic/vip-design-system). " +
    ...
    "to align with Automattic's own design language.",
    {
      section: z.enum([
        ...
        "palettes=raw hex palette | semantic=light roles | semantic-dark=dark roles | " +
      ...
      palette: z.enum([
        ...
        "Filter to a specific color palette (only applies when section includes palettes)"
      ),
    },
    async ({ section, palette }) => {
      ...
      if (showAll || section === "palettes") {
        out.palettes = palette === "all"
          ? VIP_COLORS
          : { [palette]: VIP_COLORS[palette as keyof typeof VIP_COLORS] };
      }
      ...
      return { content: [{ type: "text" as const, text: header + JSON.stringify(out, null, 2) }] };
    }
  );
  ...
  server.tool(
    "vip_design_theme_json",
    "Generate a WordPress theme.json using Automattic VIP Design System tokens. " +
    ...
    {
      style: z.enum(["default", "minimal", "full", "dark"]).default("default").describe(
        "default=brand palette + responsive type | minimal=gray/neutral only | " +
        "full=all 10 palettes + complete settings | dark=dark-mode semantic theme"
      ),
      palette_names: z.array(
        z.enum(["gold", "gray", "green", "blue", "red", "yellow", "orange", "pink", "salmon", "parsely-green"])
      ).optional().describe("Override which palettes to include (default depends on style)"),
      responsive_type: z.boolean().optional().default(true).describe(
        "Use responsive clamp() font sizes (true) or static rem values (false)"
      ),
    ...
    async ({ style, palette_names, responsive_type, include_spacing, include_shadows }) => {
      ...
      // ── choose palettes ─────────────────────────────────────────────────────
      const defaultPalettes: Record<string, (keyof typeof VIP_COLORS)[]> = {
      ...
      const chosenPalettes = (palette_names ?? defaultPalettes[style]) as (keyof typeof VIP_COLORS)[];

      // ── build color.palette array ────────────────────────────────────────────
      type WpPaletteEntry = { slug: string; color: string; name: string };
      const colorPalette: WpPaletteEntry[] = [];
