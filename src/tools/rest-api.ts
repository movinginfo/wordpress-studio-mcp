...
export function registerRestApiTools(server: McpServer): void {
  ...
  server.tool(
    "wpcom_mcp_call",
    "Proxy a raw MCP protocol call directly to the official WordPress.com MCP endpoint " +
    ...
    {
      method: z.enum(["tools/list", "tools/call", "resources/list", "resources/read"]).describe(
        "MCP protocol method"
      ),
      params: z.record(z.unknown()).optional().describe(
        "MCP params object. For tools/call: { name: 'wpcom-mcp-posts-search', arguments: { wpcom_site: '...', search: '...' } }"
      ),
    },
    async ({ method, params }) => {
      const token = getWpcomToken();
      ...
      const res = await httpRequest(
        `${WPCOM_REST_BASE}/wpcom/v2/mcp/v1`,
        "POST",
        {
          "Authorization": `Bearer ${token}`,
          "Content-Type":  "application/json",
