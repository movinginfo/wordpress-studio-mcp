...
function resolvePluginSource(data: PluginData): string | null {
  if (typeof data === "string") return data;
  if (data.resource === "wordpress.org/plugins") return data.slug;
  if (data.resource === "url") return data.url;
  return null; // file resource — skip
}

function resolveThemeSource(data: ThemeData): string | null {
  if (typeof data === "string") return data;
  if (data.resource === "wordpress.org/themes") return data.slug;
  if (data.resource === "url") return data.url;
  return null;
}
...
const FEATURED_BLUEPRINTS: Record<string, Blueprint> = {
...
export function registerBlueprintTools(server: McpServer): void {
  ...
  server.tool(
    "studio_blueprint_list",
    "List the three built-in Studio featured blueprints (Quick Start, Development, Commerce) " +
    "with their full step definitions. Use as starting points for custom blueprints.",
    {
      name: z.enum(["quick-start", "development", "commerce", "all"]).default("all").describe(
        "Which blueprint to show, or 'all' for all three"
      ),
    },
    async ({ name }) => {
  ...
  server.tool(
    "studio_blueprint_generate",
    "Snapshot an existing Studio site into a reusable Blueprint JSON. " +
    ...
    "Output can be saved to a .json file and applied to any Studio site.",
    {
      ...
      include_inactive_plugins: z.boolean().optional().default(false).describe(
        "Also include inactive plugins in the blueprint"
      ),
    },
    async ({ site, include_inactive_plugins }) => {
      ...
      let pluginSteps: BlueprintStep[] = [];
      if (pluginResult.ok && pluginResult.stdout) {
        try {
          const plugins = JSON.parse(pluginResult.stdout) as Array<{
        ...
        } catch { results.push("Warning: could not parse plugin list"); }
      } else {
      ...
      let themeSteps: BlueprintStep[] = [];
      if (themeResult.ok && themeResult.stdout) {
        try {
          const themes = JSON.parse(themeResult.stdout) as Array<{ name: string }>;
          themeSteps = themes.map(t => ({
            step:      "installTheme",
            themeData: { resource: "wordpress.org/themes", slug: t.name },
            options:   { activate: true },
          }));
        } catch { results.push("Warning: could not parse theme list"); }
      }
      ...
      for (const key of optionKeys) {
        ...
        if (r.ok && r.stdout) siteOptions[key] = r.stdout;
      }

      // ── wp-config constants ───────────────────────────────────────────────────
      const debugConsts: Record<string, boolean | string> = {};
  ...
  server.tool(
    "studio_blueprint_apply",
    "Apply a Blueprint JSON to an existing Studio site. " +
    ...
    {
      site: z.string().describe("Site name or absolute site path"),
      blueprint: z.union([z.string(), z.record(z.unknown())]).describe(
        "Blueprint JSON — either a JSON string or a parsed object"
      ),
      confirmed: z.boolean().describe(
        "Must be true — this operation installs plugins/themes and modifies the site"
      ),
    },
    async ({ site, blueprint, confirmed }) => {
      if (!confirmed) {
        return {
          content: [{ type: "text" as const, text:
            "Set confirmed: true to proceed. This will modify the target site." }],
          isError: true,
        };
      }
      ...
      try {
        bp = typeof blueprint === "string"
          ? JSON.parse(blueprint) as Blueprint
          : blueprint as Blueprint;
      } catch (err) {
        return {
          content: [{ type: "text" as const, text:
            `Invalid blueprint JSON: ${err instanceof Error ? err.message : String(err)}` }],
          isError: true,
        };
      }
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        try {
          switch (step.step) {
            ...
            // ── installPlugin ──────────────────────────────────────────────────
            case "installPlugin": {
              const source = resolvePluginSource(step.pluginData as PluginData);
              if (!source) { log.push(`${prefix} — skipped (file resource not supported)`); break; }
              const opts   = step.options as { activate?: boolean; ifAlreadyInstalled?: string } | undefined;
            ...
            // ── activatePlugin ─────────────────────────────────────────────────
            case "activatePlugin": {
              const slug = (step.pluginName ?? step.pluginPath) as string | undefined;
              if (!slug) { log.push(`${prefix} — skipped (no pluginName/pluginPath)`); break; }
            ...
            // ── installTheme ───────────────────────────────────────────────────
            case "installTheme": {
              const source = resolveThemeSource(step.themeData as ThemeData);
              if (!source) { log.push(`${prefix} — skipped (file resource not supported)`); break; }
              const opts   = step.options as { activate?: boolean } | undefined;
            ...
            // ── activateTheme ──────────────────────────────────────────────────
            case "activateTheme": {
              const slug = step.themeFolderName as string | undefined;
              if (!slug) { log.push(`${prefix} — skipped (no themeFolderName)`); break; }
            ...
            // ── setSiteOptions ─────────────────────────────────────────────────
            case "setSiteOptions": {
              const opts = step.options as Record<string, string> | undefined;
              if (!opts) { log.push(`${prefix} — skipped (no options)`); break; }
              const entries = Object.entries(opts);
              let okCount = 0;
              for (const [key, value] of entries) {
                const r = studioWp(sp, ["option", "update", key, String(value)]);
                if (r.ok) okCount++;
                else log.push(`  ${key} — FAILED: ${r.stderr}`);
              }
            ...
            case "defineWpConfigConsts": {
              const consts = step.consts as Record<string, unknown> | undefined;
              if (!consts) { log.push(`${prefix} — skipped (no consts)`); break; }
              const wpConfigPath = path.join(sp, "wp-config.php");
              if (!fs.existsSync(wpConfigPath)) {
                log.push(`${prefix} — FAILED: wp-config.php not found`); break;
              }
            ...
            case "setSiteLanguage": {
              const lang = step.language as string | undefined;
              if (!lang || lang === "en_US") {
                log.push(`${prefix} "${lang ?? "en_US"}" — skipped (default)`); break;
              }
            ...
            // ── updateUserMeta ─────────────────────────────────────────────────
            case "updateUserMeta": {
              const userId = step.userId as number | undefined;
              const meta   = step.meta   as Record<string, unknown> | undefined;
              if (!userId || !meta) { log.push(`${prefix} — skipped (missing userId or meta)`); break; }
            ...
            // ── runSql ─────────────────────────────────────────────────────────
            case "runSql": {
              const sql    = step.sql as string | undefined;
              if (!sql) { log.push(`${prefix} — skipped (no sql)`); break; }
              const dbPath = path.join(sp, "wp-content", "database", ".ht.sqlite");
              if (!fs.existsSync(dbPath)) {
                log.push(`${prefix} — FAILED: SQLite DB not found (start the site first)`); break;
              }
              try {
            ...
            case "runPHP": {
              ...
              if (!code) { log.push(`${prefix} — skipped (no code)`); break; }
            ...
            // ── wp-cli ─────────────────────────────────────────────────────────
            case "wp-cli": {
              const command = step.command as string | undefined;
              if (!command) { log.push(`${prefix} — skipped (no command)`); break; }
            ...
            // ── writeFile ──────────────────────────────────────────────────────
            case "writeFile": {
              const filePath = step.path as string | undefined;
              const data     = step.data as string | undefined;
              if (!filePath || data === undefined) {
                log.push(`${prefix} — skipped (missing path or data)`); break;
              }
              ...
              if (!absPath.startsWith(sp)) {
                log.push(`${prefix} — DENIED: path outside site root`); break;
              }
            ...
            // ── mkdir ──────────────────────────────────────────────────────────
            case "mkdir": {
              const dirPath = step.path as string | undefined;
              if (!dirPath) { log.push(`${prefix} — skipped (no path)`); break; }
            ...
            case "cp":
            case "mv": {
              const from = step.fromPath as string | undefined;
              const to   = step.toPath   as string | undefined;
              if (!from || !to) { log.push(`${prefix} — skipped (missing fromPath/toPath)`); break; }
              ...
              if (step.step === "mv") fs.unlinkSync(absFrom);
            ...
            case "importThemeStarterContent": {
              const slug = step.themeSlug as string | undefined;
              if (!slug) { log.push(`${prefix} — skipped (no themeSlug)`); break; }
              const r = studioWp(sp, ["theme", "activate", slug]);
