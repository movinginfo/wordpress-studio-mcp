 ...
 */
function patchXdebug(
  targetId:   string | null,  // null = update ALL sites (bulk disable)
  value:      boolean
): { ok: boolean; message: string } {
  try {
    const raw  = fs.readFileSync(CLI_CONFIG_PATH, "utf-8");
    const cfg  = JSON.parse(raw) as { sites: CliSite[] };
    let changed = false;
    ...
    if (!changed) return { ok: false, message: `Site id '${targetId}' not found in cli.json` };
...
function readRawSites(): CliSite[] {
  try {
    const raw = fs.readFileSync(CLI_CONFIG_PATH, "utf-8");
    return (JSON.parse(raw) as { sites: CliSite[] }).sites ?? [];
  } catch {
    return [];
  }
}
...
function getXdebugSite(): CliSite | undefined {
  return readRawSites().find(s => s.enableXdebug === true);
}

// ─── VS Code launch.json generator ───────────────────────────────────────────

function generateVsCodeLaunchJson(sitePath: string, sitePort: number): string {
...
export function registerXdebugTools(server: McpServer): void {

  // ── studio_xdebug_enable ──────────────────────────────────────────────────

  server.tool(
    "studio_xdebug_enable",
    "Enable Xdebug on a WordPress Studio site. " +
  ...
  server.tool(
    "studio_xdebug_disable",
    "Disable Xdebug on a WordPress Studio site. " +
  ...
  server.tool(
    "studio_xdebug_status",
    "Show Xdebug status across all WordPress Studio sites — which site has it enabled, " +
    "port, path mapping, and connection details for IDE setup.",
    {},
    async () => {
      const sites = readRawSites();
      if (sites.length === 0) {
        return { content: [{ type: "text" as const, text: "Cannot read ~/.studio/cli.json or no sites found" }], isError: true };
      }
      ...
      if (inactive.length > 0) {
        ...
        for (const s of inactive) {
          lines.push(`  ○ ${(s as CliSite).name}  (localhost:${(s as CliSite).port})`);
        }
      }
  ...
  server.tool(
    "studio_xdebug_ide_config",
    "Generate Xdebug IDE configuration for a Studio site. " +
